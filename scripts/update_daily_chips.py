...
def process_stock(sid):
    ...
    try:
        ...
        if not new_data['institutional_investors'] and not new_data['margin_trading']:
            return sid, False, "⚠️ 無籌碼資料 (抓取失敗或被擋)"
...
def run_daily_update(stock_list, workers=4, chunk_size=50):
    total = len(stock_list)
    print(f"📊 啟動【每日籌碼更新】排程 (多執行緒: {workers})，預計處理 {total} 檔...")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Update Daily Chips from MoneyDJ')
